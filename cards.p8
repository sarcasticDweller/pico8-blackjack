...
Card = {}
function Card:new(suit_enum, value_enum, face_down)
...
function Card.__lt(card1, card2) -- <
    if getmetatable(card1) ~= Card or getmetatable(card2) ~= Card then
        return false
    end
    return card1.value < card2.value
end
function Card.__gt(card1, card2) -- >
    if getmetatable(card1) ~= Card or getmetatable(card2) ~= Card then
        return false
    end
    return card1.value > card2.value
end
function Card.__tostring(card)
    if card.face_down
    then
        return "Back of card"
    end
